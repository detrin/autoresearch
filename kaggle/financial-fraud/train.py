...
models = [lgbm1, lgbm2, rf, et]
for m in models:
    m.fit(X_train, y_train)

preds = np.array([m.predict_proba(X_val)[:, 1] for m in models])
ranks = np.array([rankdata(p) / len(p) for p in preds])

def neg_auc(w):
    w = np.abs(w)
    w = w / w.sum()
    blend = (w[:, None] * ranks).sum(axis=0)
...
probs = (best_w[:, None] * ranks).sum(axis=0)
...
with mlflow.start_run():
    ...
    mlflow.log_param("model", "Ensemble(2xLGBM+RF+ET) rank avg")
    mlflow.log_param("description", "4-model ensemble rank averaging opt weights")
